...
class ToolSurfaceTest(unittest.TestCase):
    ...
    def test_implemented_is_capture_keyboard_clipboard_and_utility(self):
        # capture (SCRUM-1397 screenshot + SCRUM-1400 zoom) + keyboard
        # (SCRUM-1403) + clipboard/session (SCRUM-1404) + utility (SCRUM-1405).
        self.assertEqual(
            IMPLEMENTED,
            {
                "screenshot",
...
class ComputerBatchTest(unittest.TestCase):
    ...
    def test_stops_at_first_error_and_skips_the_rest(self):
        res = self._batch(
            [
                {"name": "type", "arguments": {"text": "hi"}},
                {"name": "left_click", "arguments": {"coordinate": [1, 2]}},  # pending
        ...
        self.assertTrue(
            any("SCRUM-1401" in b.get("text", "") for b in res["content"][1:])
        )
